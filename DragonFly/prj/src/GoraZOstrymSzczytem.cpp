...
/*!
 * \file 
 * \brief Definicje metod i funkcji pomocniczych  klasy GoraZOstrymSzczytem
 *
 *  Plik zawiera definicje metod i funkcji pomocniczych  klasy GoraZOstrymSzczytem.
 */

/*! 
*  Konstruktor parametryczny z wartościami domyślnymi klasy GoraZOstrymSzczytem
*/
GoraZOstrymSzczytem::GoraZOstrymSzczytem(const std::string & s1, const std::string & s2,
 ...
 */
bool GoraZOstrymSzczytem::Tworz() const
{
    ...
    for (unsigned int Ind_krawedz = 0; Ind_krawedz < 5; ++Ind_krawedz) {
        for (unsigned int Ind_wierz = 0; Ind_wierz < 4; ++Ind_wierz) {
           ...
           Wierz[X] = Wierz[Y] =0;
            ...
            Wierz = Skaluj(Wierz);
            Wierz = TransformujWierzcholek(Wierz);
            StrmWyj <<Wierz;
 ...
 * \retval false - operacja nie powiodła się.
*/
bool GoraZOstrymSzczytem::Usun()const 
{
    std::ofstream  StrmWyj(WezNazwePliku_BrylaFinalna());     
...
*/
Wektor3D GoraZOstrymSzczytem::TransformujWierzcholek(const Wektor3D& Wierz) const
 {  
...
na plaszczyzne 2D(to koło ma promień połowy długości przekątnej korpusu drona)*/
 bool GoraZOstrymSzczytem::CzyZajete(const Dron& Dn)
 {
 ...
 double GoraZOstrymSzczytem::ObliczWydluzenieTrasyPrzelotu(const Dron& Dn, std::vector<Wektor3D>& PunktySciezki)const
 {
     Wektor2D WierzcholkiProstokata[ILOSC_WIERZ_PROSTOKATA];
