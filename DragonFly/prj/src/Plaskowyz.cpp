...
/*!
 * \file 
 * \brief Definicje metod i funkcji pomocniczych  klasy Plaskowyz
 *
 *  Plik zawiera definicje metod i funkcji pomocniczych  klasy Plaskowyz.
 */

/*! 
*  Konstruktor parametryczny z wartościami domyślnymi klasy Plaskowyz
*/
Plaskowyz::Plaskowyz(const std::string & s1, const std::string & s2,
...
*
* \retval true - plik z wypełniony zerami 
 *                 został zapisany poprawnie.
 * \retval false - operacja nie powiodła się.
*/
bool Plaskowyz::Usun()const 
{
    std::ofstream  StrmWyj(WezNazwePliku_BrylaFinalna());     
...
*/
Wektor3D Plaskowyz::TransformujPolozenie(const Wektor3D& Wierz) const
 {  
     Wektor3D WierzTym = Polozenie +Wierz;
     return WierzTym;
 }
...
na plaszczyzne 2D(to koło ma promień połowy długości przekątnej korpusu drona)*/
 bool Plaskowyz::CzyZajete(const Dron& Dn)
 {
 ...
 double Plaskowyz::ObliczWydluzenieTrasyPrzelotu(const Dron& Dn, std::vector<Wektor3D>& PunktySciezki)const
 {
     Wektor2D WierzcholkiProstokata[ILOSC_WIERZ_PROSTOKATA];
 ...
 /*if((((Dn.WezPolozenieDrona()[X]+PromienKola)>=WierzcholkiProstokata[A][X])&&((Dn.WezPolozenieDrona()[Y])>=WierzcholkiProstokata[A][Y])
    &&((Dn.WezPolozenieDrona()[Y])<=WierzcholkiProstokata[D][Y])))
        std::cout<<"SIEMA1"<<std::endl;
    if((((Dn.WezPolozenieDrona()[X]-PromienKola)<=WierzcholkiProstokata[B][X])&&((Dn.WezPolozenieDrona()[Y])>=WierzcholkiProstokata[B][Y])
    &&((Dn.WezPolozenieDrona()[Y])<=WierzcholkiProstokata[C][Y])))        
        std::cout<<"SIEMA2"<<std::endl;
    if((((Dn.WezPolozenieDrona()[Y]-PromienKola)<=WierzcholkiProstokata[C][Y])&&((Dn.WezPolozenieDrona()[Y])>=WierzcholkiProstokata[A][Y])
    &&((Dn.WezPolozenieDrona()[X])>=WierzcholkiProstokata[D][X])
    &&((Dn.WezPolozenieDrona()[X])<=WierzcholkiProstokata[C][X])))        
        std::cout<<"SIEMA3"<<std::endl;
    if((((Dn.WezPolozenieDrona()[Y]+PromienKola)>=WierzcholkiProstokata[A][Y])&&((Dn.WezPolozenieDrona()[Y])<=WierzcholkiProstokata[A][Y])&&
    ((Dn.WezPolozenieDrona()[X])>=WierzcholkiProstokata[A][X])&&((Dn.WezPolozenieDrona()[X])<=WierzcholkiProstokata[B][X])))            
        std::cout<<"SIEMA4"<<std::endl;*/